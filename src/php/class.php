...
//globalref is defined here and assigned when called by ODOSession initialization. Wakeup will need to reregister each object
//to this array as they will have new addresses and references when a session is reinitialized.
global $globalref;
$globalref = array();
...
class ODOSession {
	...
	function RegisterPublicFunction($ObjectName, $ClassName, $FunctionName) {
	...
	function RegisterObject($ObjectName, &$Obj) {
		//only used for
	...
	function ConvertPostGetVars($loadmobilesite = FALSE) {
	...
	function WakeupRegisteredObject($Name) {
	...
	function BuildWakeupObjectScript($Name) {
	...
	function BuildWakeupAllObjectsScript() {
	...
	function OpenPage($mobile = FALSE) {
	...
	function CheckForRights() {
		...
		if((!isset($this->pg)) || ($this->pg == -1)) {
			...
			$query = "select * from ODOPages WHERE PageName='404'";
			$result = $GLOBALS['globalref'][1]->Query($query);
			if(!mysqli_num_rows($result)) {
			...
			$Comment = "Page Not Found: pg=" . $this->EscapedVars["pg"];
			$GLOBALS['globalref'][4]->LogEvent("404PAGENOTFOUND", $Comment, 5);
			exit(1);
		}
